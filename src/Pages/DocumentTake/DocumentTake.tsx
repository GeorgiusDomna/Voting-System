...
const DocumentTake: React.FC = () => {
  ...
  return (
    <div className={style.documentTake}>
      <h1 className={style.documentTake__title}>{t(`${Localization.DocumentTake}.title`)}</h1>
      {isLoading ? (
        <Loading type={'spinningBubbles'} color={'#bdbdbd'} />
      ) : (
        <>
          <h2 className={[style.documentTake__title, style.documentTake__subtitle].join(' ')}>
            {departamentInfo?.name} {t(`${Localization.DocumentTake}.department`)}
          </h2>

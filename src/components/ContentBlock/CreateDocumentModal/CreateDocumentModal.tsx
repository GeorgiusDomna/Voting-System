import Modal from 'react-modal';
import { observer } from 'mobx-react-lite';
import * as Yup from 'yup';
import { Form, Formik } from 'formik';
import InputText from '@/components/Auth/Inputs/InputText';
import styles from './createDocumentModal.module.css';
import closeIcon from '@/assets/cancel.svg';
import { useState } from 'react';
import InputUpload from '@/components/Shared/InputUpload/InputUpload';
import authStore from '@/stores/AuthStore';
import { createDoc, createFile } from '@/api/docuService';
import alertStore from '@/stores/AlertStore';

interface ICreateDocumentModalProps {
  toggle: () => void;
  isOpen: boolean;
}

interface values {
  docname: string;
  file: string;
}

interface valuesImages {
  image: string;
  text: string;
}

if (process.env.NODE_ENV !== 'test') Modal.setAppElement('#root');

const CreateDocumentModal: React.FC<ICreateDocumentModalProps> = observer(({ isOpen, toggle }) => {
  const [images, setImages] = useState<valuesImages[]>([]);
  const [files, setFiles] = useState<File[]>([]);
  const AddUserSchema = Yup.object().shape({
    docname: Yup.string().min(2, 'Минимум 2 символа').required('Поле обязательно для заполнения'),
    file: Yup.string().required('Загрузка файлов обязательна'),
  });

  function handleSubmit(values: values) {
    if (authStore.token) {
      createDoc(authStore.token, values.docname)
        .then((res) => {
          Promise.all(
            files.map((item) => {
              if (authStore.token) {
                return createFile(authStore.token, res.id, item);
              }
              return;
            })
          )
            .then((result) => {
              console.log(result);
            })
            .catch((error) => alertStore.toggleAlert(error));
          console.log(res);
        })
        .catch((error) => alertStore.toggleAlert(error));
    }
  }

  return (
    <Modal isOpen={isOpen} contentLabel='Модальное окно' className={styles.modal}>
      <img src={closeIcon} className={styles.modal__close} onClick={toggle} />
      <Formik
        initialValues={{
          docname: '',
          file: '',
        }}
        validateOnMount
        validateOnChange
        validationSchema={AddUserSchema}
        onSubmit={handleSubmit}
      >
        {({ handleChange, values, isValid, errors, submitCount }) => (
          <Form className={styles.form} name='createDoc'>
            <InputText
              type='text'
              name='docname'
              placeholder='Название документа'
              value={values.docname}
              error={errors.docname}
              handleChange={handleChange}
              submitCount={submitCount}
            />
            <ul
              className={[styles.listImage, `${images.length > 3 && styles.listImage_scroll}`].join(
                ' '
              )}
            >
              {images.length > 0 &&
                images.map((item, index) => (
                  <li key={index} className={styles.image}>
                    <img className={styles.preview} src={item.image} alt={item.text} />
                  </li>
                ))}
            </ul>
            <InputUpload
              name='file'
              placeholder='Название документа'
              value={values.file}
              error={errors.file}
              handleChange={handleChange}
              submitCount={submitCount}
              images={images}
              setImages={setImages}
              filesImages={files}
              setFiles={setFiles}
            />
            <button type='submit' className={styles.button} disabled={submitCount >= 1 && !isValid}>
              Добавить документ
            </button>
          </Form>
        )}
      </Formik>
    </Modal>
  );
});

export default CreateDocumentModal;
